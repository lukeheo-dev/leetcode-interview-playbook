...
def read_title(notes: Path) -> tuple[int, str]:
    ...
    if not m:
        raise ValueError(f"Invalid title line in {notes}: {first_line!r}")
...
def main() -> None:
    ...
    notes_files = sorted(Path("problems").glob("**/lc_*/notes.md"))
    ...
    # Latest: newest commit first
    latest = sorted(problems, key=lambda p: (p.ts, p.lc), reverse=True)[:10]
    latest_body = "".join([f"- [LC {p.lc:04d} — {p.title}]({p.notes_path})\n" for p in latest])

    # Problems: LC number ascending
    all_sorted = sorted(problems, key=lambda p: p.lc)
    problems_body = "".join([f"- [LC {p.lc:04d} — {p.title}]({p.notes_path})\n" for p in all_sorted])
